...
extern vector <double> Hamming;            // Hamming window vector
extern float FiltWeight[FiltNum][FFTLen/2+1]; //This is the Mel filterbank weights
extern vector<float> Coeff; // This stores cepstrum and log energy
...
/*int main()
{
	...
	if (HIGH_FREQ>(int) (FS/2))                      // Check pre-defined high frequency
	   HIGH_FREQ=(int) (FS/2);
	if (LOW_FREQ>HIGH_FREQ)                               // Check pre-defined low frequency
	   LOW_FREQ=(int)(HIGH_FREQ/2);
	...
	CreateFilt(FiltWeight, FiltNum, FS*1000, HIGH_FREQ*1000, LOW_FREQ*1000);    // Compute filter weights
