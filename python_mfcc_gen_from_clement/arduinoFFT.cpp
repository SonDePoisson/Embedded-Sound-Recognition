...
arduinoFFT::arduinoFFT(double *vReal, double *vImag, u_int16_t samples, double samplingFrequency)
{// Constructor
	this->_vReal = vReal;
	this->_vImag = vImag;
...
u_int8_t arduinoFFT::Revision(void)
{
	return(FFT_LIB_REV);
}

void arduinoFFT::Compute(double *vReal, double *vImag, u_int16_t samples, u_int8_t dir)
{
	// #warning("This method is deprecated and may be removed on future revisions.")
	Compute(vReal, vImag, samples, Exponent(samples), dir);
}

void arduinoFFT::Compute(u_int8_t dir)
{// Computes in-place complex-to-complex FFT /
	// Reverse bits /
	u_int16_t j = 0;
	for (u_int16_t i = 0; i < (this->_samples - 1); i++) {
		if (i < j) {
			Swap(&this->_vReal[i], &this->_vReal[j]);
			if(dir==FFT_REVERSE)
				Swap(&this->_vImag[i], &this->_vImag[j]);
		}
		u_int16_t k = (this->_samples >> 1);
		while (k <= j) {
			j -= k;
			k >>= 1;
		}
	...
	double c2 = 0.0;
	u_int16_t l2 = 1;
	for (u_int8_t l = 0; (l < this->_power); l++) {
		u_int16_t l1 = l2;
		l2 <<= 1;
		double u1 = 1.0;
		double u2 = 0.0;
		for (j = 0; j < l1; j++) {
			 for (u_int16_t i = j; i < this->_samples; i += l2) {
					u_int16_t i1 = i + l1;
	...
	if (dir != FFT_FORWARD) {
		for (u_int16_t i = 0; i < this->_samples; i++) {
...
void arduinoFFT::Compute(double *vReal, double *vImag, u_int16_t samples, u_int8_t power, u_int8_t dir)
{	// Computes in-place complex-to-complex FFT
	// Reverse bits
	// #warning("This method is deprecated and may be removed on future revisions.")
	u_int16_t j = 0;
	for (u_int16_t i = 0; i < (samples - 1); i++) {
		if (i < j) {
			Swap(&vReal[i], &vReal[j]);
			if(dir==FFT_REVERSE)
				Swap(&vImag[i], &vImag[j]);
		}
		u_int16_t k = (samples >> 1);
		while (k <= j) {
			j -= k;
			k >>= 1;
		}
	...
	double c2 = 0.0;
	u_int16_t l2 = 1;
	for (u_int8_t l = 0; (l < power); l++) {
		u_int16_t l1 = l2;
		l2 <<= 1;
		double u1 = 1.0;
		double u2 = 0.0;
		for (j = 0; j < l1; j++) {
			 for (u_int16_t i = j; i < samples; i += l2) {
					u_int16_t i1 = i + l1;
...
void arduinoFFT::ComplexToMagnitude(double *vReal, double *vImag, u_int16_t samples)
{	// vM is half the size of vReal and vImag
	// #warning("This method is deprecated and may be removed on future revisions.")
	for (u_int16_t i = 0; i < samples; i++) {
...
void arduinoFFT::DCRemoval(double *vData, u_int16_t samples)
{
	// calculate the mean of vData
	// #warning("This method is deprecated and may be removed on future revisions.")
	double mean = 0;
...
void arduinoFFT::Windowing(u_int8_t windowType, u_int8_t dir)
...
void arduinoFFT::Windowing(double *vData, u_int16_t samples, u_int8_t windowType, u_int8_t dir)
{// Weighing factors are computed once before multiple use of FFT
// The weighing function is symetric; half the weighs are recorded
	// #warning("This method is deprecated and may be removed on future revisions.")
...
{
	double maxY = 0;
	u_int16_t IndexOfMaxY = 0;
	//If sampling_frequency = 2 * max_frequency in signal,
	//value would be stored at position samples/2
	for (u_int16_t i = 1; i < ((this->_samples >> 1) + 1); i++) {
...
double arduinoFFT::MajorPeak(double *vD, u_int16_t samples, double samplingFrequency)
{
	// #warning("This method is deprecated and may be removed on future revisions.")
	double maxY = 0;
	u_int16_t IndexOfMaxY = 0;
	//If sampling_frequency = 2 * max_frequency in signal,
	//value would be stored at position samples/2
	for (u_int16_t i = 1; i < ((samples >> 1) + 1); i++) {
...
void arduinoFFT::MajorPeak(double *vD, u_int16_t samples, double samplingFrequency, double *f, double *v)
{
	// #warning("This method is deprecated and may be removed on future revisions.")
	double maxY = 0;
	u_int16_t IndexOfMaxY = 0;
...
u_int8_t arduinoFFT::Exponent(u_int16_t value)
{
