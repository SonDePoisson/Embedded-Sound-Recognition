...
public:
	/* Constructor */
	arduinoFFT(void);
	arduinoFFT(double *vReal, double *vImag, u_int16_t samples, double samplingFrequency);
	/* Destructor */
	~arduinoFFT(void);
	/* Functions */
	u_int8_t Revision(void);
	u_int8_t Exponent(u_int16_t value);

	void ComplexToMagnitude(double *vReal, double *vImag, u_int16_t samples);
	void Compute(double *vReal, double *vImag, u_int16_t samples, u_int8_t dir);
	void Compute(double *vReal, double *vImag, u_int16_t samples, u_int8_t power, u_int8_t dir);
	void DCRemoval(double *vData, u_int16_t samples);
	double MajorPeak(double *vD, u_int16_t samples, double samplingFrequency);
	void MajorPeak(double *vD, u_int16_t samples, double samplingFrequency, double *f, double *v);
	void Windowing(double *vData, u_int16_t samples, u_int8_t windowType, u_int8_t dir);

	void ComplexToMagnitude();
	void Compute(u_int8_t dir);
	void DCRemoval();
	double MajorPeak();
	void MajorPeak(double *f, double *v);
	void Windowing(u_int8_t windowType, u_int8_t dir);
